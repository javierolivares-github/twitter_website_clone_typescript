...
describe('NotificationIcon', () => {
  ...
  test('The component should render a svg element.', () => {
    render(
      <NotificationIcon fillColor="text-black-900" isFilled={false} width="w-[1.125rem]" height="h-[1.125rem]" />
    );
    const svgElement = screen.getByTestId('notification-outlined-svg');
  ...
  test('The path element should have a fill attribute with the value of black', () => {
    ...
    const pathElement = screen.getByTestId('notification-outlined-path');
  ...
  test('If the isFilled prop is false, should renders the outlined version of the component', () => {
    ...
    const svgElement = screen.getByTestId('notification-outlined-svg');
    expect(svgElement).toHaveAttribute('data-testid', 'notification-outlined-svg');
  });
